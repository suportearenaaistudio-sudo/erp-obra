...
export class SaasAuthGuard {
    ...
    async check(userId: string, email: string): Promise<void> {
        ...
        if (error || !saasUser) {
            ...
            // Emitir evento de falha de autenticação SaaS
            LogSafe.emit(SecurityEventType.LOGIN_FAILED, {
                actorType: ActorType.SAAS_USER,
                actorId: userId,
                metadata: { reason: 'not_saas_admin', email },
            });
        ...
        LogSafe.emit(SecurityEventType.LOGIN_SUCCESS, {
            actorType: ActorType.SAAS_USER,
            actorId: userId,
            metadata: { email, role: saasUser.role },
        });
